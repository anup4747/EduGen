...
# Use Atlas connection string in backend/.env, for example:
# mongodb+srv://<username>:<password>@<cluster-url>/<db-name>?retryWrites=true&w=majority
# Passwords with reserved characters like @ must be percent-encoded.
# Alternatively set MONGO_USER / MONGO_PASSWORD / MONGO_HOST and let the code build the URI.
_mongo_db_name = os.getenv("MONGO_DB_NAME", "learnpath_db")


def _build_mongo_uri():
    raw_uri = os.getenv("MONGO_URI", "mongodb://localhost:27017").strip()
    if raw_uri:
        parsed = urlparse(raw_uri)
        username = parsed.username
        password = parsed.password
        if username and password:
            quoted_user = quote_plus(username)
            quoted_pass = quote_plus(password)
            host = parsed.hostname or ""
            port = f":{parsed.port}" if parsed.port else ""
            path = parsed.path or ""
            query = f"?{parsed.query}" if parsed.query else ""
            return f"{parsed.scheme}://{quoted_user}:{quoted_pass}@{host}{port}{path}{query}"
        return raw_uri
    ...
    if mongo_user and mongo_password:
        return (
            ...
            f"{_mongo_db_name}?retryWrites=true&w=majority"
        )
