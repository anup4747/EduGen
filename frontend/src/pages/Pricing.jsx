...
const PricingCard = ({ title, price, description, features, buttonText, highlighted, icon: Icon, accentColor }) => {
  return (
    <div className={`relative flex flex-col p-8 rounded-[2.5rem] border transition-all duration-500 group ${
      ...
      <div className="mb-8">
        <div className={`w-14 h-14 rounded-2xl flex items-center justify-center mb-6 transition-transform duration-500 group-hover:scale-110 ${accentColor}`}>
          <Icon size={28} />
        </div>
      ...
      <div className="flex-1 space-y-4 mb-10">
        {features.map((feature, idx) => (
          <div key={idx} className="flex items-start gap-3 group/item">
            <div className={`mt-1 shrink-0 ${feature.included ? "text-vscode-accent" : "text-slate-600"}`}>
              {feature.included ? <Check size={18} /> : <X size={18} />}
            </div>
            <span className={`text-sm font-medium transition-colors ${feature.included ? "text-slate-200" : "text-slate-500"}`}>
              {feature.text}
            </span>
          </div>
        ))}
      ...
      <button className={`w-full py-4 rounded-2xl font-bold transition-all duration-300 active:scale-95 ${
        highlighted
  ...
  const plans = [
    {
      title: "Free",
      price: "Free",
      description: "Perfect for casual learners and beginners exploring new topics.",
      icon: Brain,
    ...
    },
    {
      title: "Power",
      price: "$49",
      description: "The ultimate learning experience with infinite possibilities.",
      icon: Sparkles,
      ...
      <div className="relative z-10 max-w-7xl mx-auto px-6 pt-24">
        ...
        <div className="text-center mb-20 animate-fade-in-up">
          <div className="inline-flex items-center gap-2 px-4 py-2 rounded-full border border-vscode-accent/30 bg-slate-900/50 backdrop-blur-sm mb-6">
            ...
            <span className="text-sm font-semibold text-slate-300">Simple, Transparent Pricing</span>
          </div>
          <h1 className="text-4xl md:text-6xl font-bold text-white mb-6 tracking-tight">
            Choose Your <span className="bg-clip-text text-transparent bg-gradient-to-r from-vscode-accent to-purple-400">Learning Speed</span>
          </h1>
          <p className="text-slate-400 max-w-2xl mx-auto text-lg font-medium leading-relaxed">
            From casual exploration to professional mastery, we have a plan designed to help you learn anything faster with AI.
          </p>
        </div>
        ...
        <div className="mt-32 text-center animate-fade-in-up" style={{ animationDelay: '600ms' }}>
          <h2 className="text-2xl font-bold text-white mb-12">Frequently Asked Questions</h2>
          <div className="grid md:grid-cols-2 gap-6 text-left max-w-4xl mx-auto">
            {[
              { q: "Can I switch plans anytime?", a: "Yes, you can upgrade or downgrade your plan at any time from your profile settings." },
              { q: "Is there a student discount?", a: "The Free plan is designed specifically for students, but we offer seasonal discounts for Pro and Power plans." },
              { q: "What is AI Memory?", a: "AI Memory allows the chatbot to remember concepts from your other courses to provide unified cross-topic explanations." },
              { q: "What is Prep Mode?", a: "A specialized AI mode that generates intensive drills, flashcards, and practice exams for competitive certification prep." }
            ].map((faq, i) => (
              <div key={i} className="p-6 rounded-2xl bg-slate-900/40 border border-slate-800/60 hover:border-slate-700 transition-colors">
        ...
        <div className="mt-24 text-center">
            <Link to="/" className="text-slate-500 hover:text-vscode-accent text-sm font-medium transition-colors">
                ← Back to Home
            </Link>
        </div>
      </div>
