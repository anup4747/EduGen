...
function levelBadgeClass(level) {
  if (level === "Beginner") return "bg-emerald-500/10 border-emerald-500/20 text-emerald-400";
  if (level === "Intermediate") return "bg-amber-500/10 border-amber-500/20 text-amber-400";
    ...
    <div className="flex min-h-[calc(100vh-3.5rem)] bg-[#080b12] text-white font-['Inter'] selection:bg-vscode-accent selection:text-white relative overflow-hidden">
        ...
        <main className="flex-1 w-full max-w-6xl mx-auto px-6 py-10 lg:px-10 lg:py-12 z-10 custom-scrollbar overflow-y-auto">
            ...
            <div className="flex items-center justify-between mb-8 animate-[fade-in-up_0.4s_ease-out_forwards]">
                ...
                >
                    <Settings size={16} className="text-vscode-accent" /> Account Settings
                </button>
            </div>
            ...
            <div className="space-y-8 pb-12">
                ...
                <section className="relative rounded-[2.5rem] bg-slate-950/60 p-8 lg:p-10 ring-1 ring-slate-800/80 shadow-2xl backdrop-blur-xl overflow-hidden animate-[fade-in-up_0.5s_ease-out_forwards] border border-slate-800/50">
                    ...
                    <div className="relative flex flex-col gap-8 lg:flex-row lg:items-center">
                        <div className="relative group shrink-0">
                           ...
                           <button onClick={() => setIsEditing(true)} className="absolute bottom-0 right-0 p-2.5 bg-vscode-accent rounded-full text-white shadow-lg border-2 border-[#080b12] hover:scale-110 transition-transform">
                              <Edit2 size={14} />
                           </button>
                        </div>
                        ...
                        <div className="flex-1">
                          ...
                          <p className="text-lg text-slate-400 font-medium font-mono mb-4">@{profileHandle}</p>
                ...
                <section className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
                    <div className="rounded-[2rem] bg-slate-900/40 p-6 ring-1 ring-slate-800 transform hover:-translate-y-1 transition-all duration-300 hover:shadow-[0_0_25px_rgba(124,58,237,0.15)] hover:bg-slate-900/60 animate-[fade-in-up_0.6s_ease-out_forwards] border-t border-t-slate-800/80">
                        <div className="flex items-center gap-3 mb-4">
                           ...
                           <p className="text-xs uppercase tracking-[0.24em] font-bold text-slate-400">Total Topics</p>
                        </div>
                        <p className="text-4xl font-bold text-white tracking-tight">{topics.length}</p>
                    </div>
                    ...
                    <div className="rounded-[2rem] bg-slate-900/40 p-6 ring-1 ring-slate-800 transform hover:-translate-y-1 transition-all duration-300 hover:shadow-[0_0_25px_rgba(16,185,129,0.15)] hover:bg-slate-900/60 animate-[fade-in-up_0.7s_ease-out_forwards] border-t border-t-slate-800/80">
                        <div className="flex items-center gap-3 mb-4">
                           ...
                           <p className="text-xs uppercase tracking-[0.24em] font-bold text-slate-400">Completed</p>
                        </div>
                        <p className="text-4xl font-bold text-white tracking-tight">{completedTopics}</p>
                    </div>
                    ...
                    <div className="rounded-[2rem] bg-slate-900/40 p-6 ring-1 ring-slate-800 transform hover:-translate-y-1 transition-all duration-300 hover:shadow-[0_0_25px_rgba(245,158,11,0.15)] hover:bg-slate-900/60 animate-[fade-in-up_0.8s_ease-out_forwards] border-t border-t-slate-800/80">
                        <div className="flex items-center gap-3 mb-4">
                           ...
                           <p className="text-xs uppercase tracking-[0.24em] font-bold text-slate-400">Best Streak</p>
                        </div>
                        <div className="flex items-baseline gap-1">
                           <p className="text-4xl font-bold text-white tracking-tight">{userAnalytics?.best_streak ?? 0}</p>
                    ...
                    <div className="rounded-[2rem] bg-slate-900/40 p-6 ring-1 ring-slate-800 transform hover:-translate-y-1 transition-all duration-300 hover:shadow-[0_0_25px_rgba(14,165,233,0.15)] hover:bg-slate-900/60 animate-[fade-in-up_0.9s_ease-out_forwards] border-t border-t-slate-800/80">
                        <div className="flex items-center gap-3 mb-4">
                           ...
                           <p className="text-xs uppercase tracking-[0.24em] font-bold text-slate-400">Study Time</p>
                        </div>
                        <div className="flex items-baseline gap-1">
                           <p className="text-4xl font-bold text-white tracking-tight">
                              {userAnalytics?.total_time ? Math.floor(userAnalytics.total_time / 60) : 0}
                           </p>
                           <span className="text-sm font-medium text-slate-400 mb-1">mins</span>
                        </div>
                    </div>
                </section>
                ...
                <section className="rounded-[2.5rem] bg-slate-950/60 p-8 lg:p-10 ring-1 ring-slate-800/80 shadow-xl backdrop-blur-sm animate-[fade-in-up_1s_ease-out_forwards]">
                    <div className="mb-8 flex items-center justify-between pb-4 border-b border-slate-800">
                        <div>
                           <h2 className="text-2xl font-bold text-white tracking-tight">Path History</h2>
                           <p className="text-sm font-medium text-slate-400 mt-1">Review your generated knowledge bases</p>
                        </div>
                    </div>
                    ...
                    {loading ? (
                      ...
                      <div className="rounded-2xl bg-rose-500/10 border border-rose-500/20 p-5 text-rose-400 text-sm font-medium">{error}</div>
                    ) : topics.length === 0 ? (
                      <div className="flex flex-col items-center justify-center p-10 bg-slate-900/30 rounded-3xl border border-slate-800/50 border-dashed">
                         ...
                         <p className="text-slate-400 font-medium">No learning paths found yet.</p>
                         <Link to="/new-topic" className="mt-4 text-vscode-accent font-bold hover:text-white transition-colors">Start your first course →</Link>
                      </div>
                    ) : (
                      <div className="grid gap-4 sm:grid-cols-2">
                        {topics.map((topic) => (
                          <div key={topic._id} className="group rounded-3xl bg-slate-900/50 p-6 border border-slate-800/80 hover:bg-slate-900 hover:border-slate-700 transition-all duration-300">
                            <div className="flex flex-col gap-4">
                              <div className="flex justify-between items-start gap-4">
                                <h3 className="text-lg font-bold text-white group-hover:text-vscode-accent transition-colors leading-tight">{topic.topic_name}</h3>
                                <span className={`shrink-0 rounded-full px-3 py-1 text-[10px] uppercase font-bold border ${levelBadgeClass(topic.level)}`}>
                                  {topic.level}
                                </span>
                              </div>
                              <div className="flex items-center justify-between text-sm mt-2 pt-4 border-t border-slate-800/50">
                                 ...
                                 <div className="flex items-center gap-2">
                                     <span className={`text-xs font-bold uppercase tracking-wider ${topic.completed ? 'text-emerald-400' : 'text-slate-500'}`}>
        ...
        <aside className={`fixed inset-y-0 right-0 z-50 w-full max-w-md bg-[#080b12] border-l border-slate-800 shadow-[0_0_80px_rgba(0,0,0,0.8)] custom-scrollbar overflow-y-auto flex flex-col transform transition-transform duration-300 ease-in-out ${isEditing ? 'translate-x-0' : 'translate-x-full'}`}>
                ...
                <form onSubmit={handleSaveProfile} className="p-8 space-y-6 flex-1">
                  ...
                  <div className="flex flex-col items-center justify-center p-6 bg-slate-900/50 rounded-3xl border border-slate-800 border-dashed mb-8">
                     ...
                     <span className="text-sm font-bold text-white mb-2">Profile Picture</span>
                  ...
                  {formError && (
                    <div className="rounded-xl bg-rose-500/10 p-3 border border-rose-500/20">
                       <p className="text-sm font-medium text-rose-400 text-center">{formError}</p>
                    </div>
                  )}
                  {success && (
                    <div className="rounded-xl bg-emerald-500/10 p-3 border border-emerald-500/20">
                       <p className="text-sm font-medium text-emerald-400 text-center">{success}</p>
                    </div>
                  )}
